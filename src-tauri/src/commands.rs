...
use crate::{
    ...
    models::login::Login, services::{login_service::insert_login, master_key_service::check_key}, utils::pass_manager::{encrypt_passwd, get_plain_credentials},
};
...
pub async fn get_logins() -> Result<Vec<Login>, String> {
    ...
    let logins = get_plain_credentials(master_key.as_str()).await.map_err(|_| Into::<String>::into("Error getting logins"));
    if logins.is_err() {
...
pub async  fn add_login(login: Login) -> Result<(), String> {
    ...
    let res = insert_login(login).await.map_err(|_| Into::<String>::into("Error inserting login"));
    if res.is_err() {
