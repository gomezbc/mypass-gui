import { c as create_ssr_component, d as compute_rest_props, g as getContext, f as spread, h as escape_attribute_value, i as escape_object, j as add_attribute, v as validate_component, e as escape } from "../../chunks/ssr.js";
...
const Connection = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let uri = "";
  let resMsg = "";
  let connectBtnMsg = "Connect";
  return `<div class="w-8/12 max-w-[700px] mt-0 bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-gray-800 dark:border-gray-700"><div class="p-6 space-y-4 md:space-y-6 sm:p-8"><h1 class="text-xl font-bold leading-tight tracking-tight md:text-2xl text-gray-800 dark:text-white" data-svelte-h="svelte-10ljo4f">Paste your mongodb URI</h1> <div class="space-y-4 md:space-y-6"><div><label for="uri" class="block mb-2 text-sm font-medium text-black dark:text-white" data-svelte-h="svelte-fqmg4k">Your URI</label> <input type="text" name="uri" id="uri" class="py-3 px-4 block w-full border-gray-200 border-2 rounded-lg text-sm text-black/70 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600" placeholder="mongodb://[username]:[password]@[host]:[port]/" ${"required"}${add_attribute("value", uri, 0)}></div> <div class="flex justify-center">${validate_component(Button, "Button").$$render(
    $$result,
    {
      color: "blue",
      class: "w-full mx-2",
    ...
    {
      default: () => {
        return `${``} ${escape(connectBtnMsg)}`;
      }
    }
  )}</div> <p class="text-center h-2 text-black/70 dark:text-white font-semibold">${escape(resMsg)}</p></div></div></div>`;
});
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let { title } = $$props;
  if ($$props.title === void 0 && $$bindings.title && title !== void 0)
    $$bindings.title(title);
  return `<!DOCTYPE html> <html lang="en"><head><meta charset="UTF-8"> <meta name="description" content="Application for handling credentials with MongoDB"> <meta name="viewport" content="width=device-width"> <link rel="icon" type="image/svg+xml" href="/favicon.svg"> <meta name="generator"${add_attribute("content", process.env.KIT, 0)}> <title>${escape(title)}</title></head> <body class="relative dark:bg-slate-900 bg-gray-100 flex h-screen items-center py-16">${slots.default ? slots.default({}) : ``} <style data-svelte-h="svelte-1iipyv4">:root {
			...
			}</style> <script src="https://cdnjs.cloudflare.com/ajax/libs/flowbite/1.8.0/flowbite.min.js" data-svelte-h="svelte-1144lzv"><\/script></body></html>`;
});
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `${validate_component(Layout, "Layout").$$render($$result, { title: "Welcome to mypass-gui" }, {}, {
    default: () => {
      return `<main class="w-screen h-screen flex flex-col items-center justify-center mx-auto md:h-screen lg:py-0"><a href="#" class="flex items-center mb-6 text-2xl font-semibold text-black dark:text-white" data-svelte-h="svelte-nfui4y"><img class="w-8 h-8 mr-2" src="https://flowbite.s3.amazonaws.com/blocks/marketing-ui/logo.svg" alt="logo">
			MyPass</a>  ${validate_component(Connection, "Connection").$$render($$result, {}, {}, {})}</main>`;
    }
  })}`;
});
export {
